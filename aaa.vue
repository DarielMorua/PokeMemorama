...
<template>
  ...
  <p>
    Para mostrar al pokemon detrás del Ditto da click en el pokemon deseado, después elige otra
  ...
  <button v-show="userName !== ''" @click="getUser">Registro/Ingreso</button>
  ...
  <div v-show="userName !== '' && gameRegion !== ''">
    ...
    <button
      ...
      v-on:click="setMode(option)"
      :class="{ selected: gameMode === option }"
    >
      {{ option }}
    </button>
  </div>
